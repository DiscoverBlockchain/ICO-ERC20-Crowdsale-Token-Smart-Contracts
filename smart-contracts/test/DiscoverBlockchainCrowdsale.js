...
contract('DiscoverBlockchainCrowdsale', function (accounts) {
    it('should deploy the token and store the address', function (done) {
        DiscoverBlockchainCrowdsale.deployed().then(async function (instance) {
    ...
    it('one ETH should buy 5 DiscoverBlockchain Tokens in PreICO', function (done) {
        DiscoverBlockchainCrowdsale.deployed().then(async function (instance) {
            const data = await instance.sendTransaction({from: accounts[7], value: web3.toWei(1, "ether")});
            const tokenAddress = await instance.token.call();
            const discoverBlockchainToken = DiscoverBlockchainToken.at(tokenAddress);
            const tokenAmount = await discoverBlockchainToken.balanceOf(accounts[7]);
    ...
    it('should set variable `totalWeiRaisedDuringPreICO` correctly', function (done) {
        DiscoverBlockchainCrowdsale.deployed().then(async function (instance) {
            let amount = await instance.totalWeiRaisedDuringPreICO.call();
    ...
    it('one ETH should buy 2 DiscoverBlockchain Tokens in ICO', function (done) {
        DiscoverBlockchainCrowdsale.deployed().then(async function (instance) {
            const data = await instance.sendTransaction({from: accounts[2], value: web3.toWei(1.5, "ether")});
            const tokenAddress = await instance.token.call();
            const discoverBlockchainToken = DiscoverBlockchainToken.at(tokenAddress);
            const tokenAmount = await discoverBlockchainToken.balanceOf(accounts[2]);
    ...
    it('should transfer the raised ETH to RefundVault during ICO', function (done) {
        DiscoverBlockchainCrowdsale.deployed().then(async function (instance) {
            let vaultAddress = await instance.vault.call();
